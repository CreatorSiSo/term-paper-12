...
Types in Rym are generally declared by the "type" keyword.
The keyword can be followed by some syntax that defines what data value of this type holds.
All types in Rym can be described by a concept called algebraic data types _(ADT/ADTs)_.
This model associates each type with the amount of values it can represent.

<!-- ### Unit Type -->
### Unit

The unit type comes from the functional languages but is very similar to the void type from C, C++, JavaScript, etc.
It has exactly one value and is the value that functions and expressions without a result return.
...
They only have two possible values, "true" and "false".
...
The was named after _George Boole_ who pioneered the field of mathematical logic.
...
: __TODO__ Supported integer formats {#tbl-int-formats tbl-colwidths="[16,10,16,23,19,16]"}

The cstdint header, Go and Rust have names for these types that clearly indicate how large a value is.
This is why Rym will just Rust's version as it is the shortest one.
They also allow for the possiblity to add integers of arbitrary size.
...
: __TODO__ Supported _IEEE-754_ floating point formats {#tbl-float-formats tbl-colwidths="[16,12,10,8,10,10,12,12,10]"}
